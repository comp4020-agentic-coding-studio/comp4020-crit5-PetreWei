...
This is a static site written in HTML, CSS and TypeScript. It builds to plain HTML, CSS and JS and deploys to GitHub Pages. **The deployed site is what gets marked**, not this repo, and not "it works on my machine". A marker opens the live URL in Chrome at two sizes, 1920x1080 (desktop) and 390x844 (phone), and drives it by keyboard alone: Tab order, arrow keys, Enter or Space, as readily as by mouse. All three count in full, and none of them show up in a green `vitest` run.
...
- Markdown paragraphs are one line each, with no hard wrap at 80 columns. Nothing here reformats prose, so a rewrap shows as a diff on every line of a paragraph and buries the sentence that actually changed.
- **Push every commit as you make it, without asking.** An unpushed commit is work that doesn't count. When `pnpm check` is green, commit, then push. Never force. Read `git status` and stage files by name, since this repo is public and a push is permanent. Then read the CI run, because a local green is not a green deploy.

## The link-preview card

`public/card.png` (1200x630) is what a shared link shows. `src/layouts/Layout.astro` is the one place that points at it, so every page that uses the shared layout gets the card for free. Replace the image and pass a `description` prop, instead of copying a head block per page. The card URL resolves against the deployed page, like any link, and nothing in CI checks it, so the deployed head is the only place a broken one shows up.
