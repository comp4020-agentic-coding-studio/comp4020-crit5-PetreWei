...
- **Never edit `.github/workflows/`.** It's the sensor this work has to satisfy, not part of the work.
...
- **A stuck automated test is evidence to investigate, not a verdict on the app.** A keyboard-only Playwright round once looked stuck after one blocker. The app was fine. The script assumed the wrong blocker count and didn't know the hint withholds the last cell.
